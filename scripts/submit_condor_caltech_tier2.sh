...
eos_dir=/store/group/lpchbb/LLDJntuples/analizer/2018_LLDJ_V2p0
...
#####################################
cp ${scripts_dir}/run_job.sh ${task_name_dir}/run_job.sh
...
for i in `ls ${task_name_dir}`; do
	...
	printf "\n" >> ${condor_submit_file}
	printf "Output = "${task_name_dir}"/out/"${executable}".\$(Cluster)_\$(Process).stdout\n" >> ${condor_submit_file}
	printf "Error  = "${task_name_dir}"/err/"${executable}".\$(Cluster)_\$(Process).stderr\n" >> ${condor_submit_file}
	printf "Log    = "${task_name_dir}"/log/"${executable}".\$(Cluster)_\$(Process).log\n" >> ${condor_submit_file}

	printf "\n" >> ${condor_submit_file}
	printf "Arguments = ${i} displaced_jet_${task_name}_${ctr}.root \n" >> ${condor_submit_file}
	printf "Queue\n" >> ${condor_submit_file}
